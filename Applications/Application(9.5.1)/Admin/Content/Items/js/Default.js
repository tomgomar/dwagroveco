...
document.addEventListener("DOMContentLoaded", (event) => {
    document.querySelectorAll('.js-field-group').forEach((table) => {
        let tabs = [];
        table.querySelectorAll('.js-tab-content').forEach((tabContent) => {
            if (tabContent.closest('.js-field-group') == table) {
                const tabName = tabContent.getAttribute('data-tabname');
                const tr = tabContent.closest('.js-tab-tr');
        ...
        if (tabs.length > 0) {
            ...
            tabs.forEach((tabName) => {
                ...
                tabLabel.addEventListener('click', (event) => {
                    table.parentNode.querySelectorAll('.js-tab-content').forEach((tab) => {
